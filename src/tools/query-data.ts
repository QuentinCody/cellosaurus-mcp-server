...
interface QueryEnv {
    CELLOSAURUS_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("CELLOSAURUS_DATA_DO", "cellosaurus");

    server.registerTool(
        "cellosaurus_query_data",
        {
            title: "Query Staged Cellosaurus Data",
            description:
                "Query staged Cellosaurus data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
