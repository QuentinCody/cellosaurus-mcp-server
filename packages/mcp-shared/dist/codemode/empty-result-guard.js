...
export class EmptyDatasetError extends Error {
    code = "EMPTY_DATASET_SUSPECTED";
    constructor(message) {
        super(message);
        this.name = "EmptyDatasetError";
    }
}
const VERIFIED_NOTE = "Filtered query returned 0 rows; an unfiltered probe confirms the dataset is live. " +
...
const RECOVERED_NOTE = "First response was an empty payload but a retry returned rows — transient upstream " +
...
async function tryRetry(options) {
    const { refetch, isEmpty, describe = "dataset", retryDelayMs = 0, log, } = options;
    if (!refetch)
        return null;
    if (retryDelayMs > 0) {
    ...
    if (retried === undefined || isEmpty(retried))
        return null;
 ...
 */
export async function guardEmptyResult(first, options) {
    ...
    const probeEmpty = probePayload === undefined || (isProbeEmpty ?? isEmpty)(probePayload);
    if (probeEmpty) {
        log?.(`dataset_empty_suspected: ${describe} — unfiltered probe returned 0 rows`);
        throw new EmptyDatasetError(`Empty result could NOT be verified: ${describe} returned no rows even for an unfiltered ` +
...
//# sourceMappingURL=empty-result-guard.js.map