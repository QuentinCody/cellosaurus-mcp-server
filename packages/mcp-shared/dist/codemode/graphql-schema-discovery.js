 ...
 */
export const SCHEMA_DISCOVERY_UNAVAILABLE = "Schema discovery is unavailable — this API has GraphQL introspection disabled. " +
 ...
 * either way.
 */
export async function ensureIntrospectionCached(cache, gqlFetch) {
    if (cache.introspection || cache.introspectionUnavailable)
        return;
    try {
 ...
 */
function catalogSearchResponse(catalog, query, maxResults, apiName) {
...
function unavailableResponse(query, apiName) {
    return {
 ...
 */
export function registerGraphqlSearchTool(server, opts) {
    const { prefix, apiName, gqlFetch, cache, catalog } = opts;
    server.tool(`${prefix}_search`, `Search the ${apiName} GraphQL schema for the query roots, types, and fields matching your keywords — so you write a ${prefix}_execute query with REAL field names instead of guessing. Call this FIRST when you don't already know the schema. Returns top-level entry points plus matching Type.field signatures with their arguments and return types.`, {
        query: z
            .string()
            .describe('Keywords to find in the schema — a gene, disease, drug, entity, or a field name like "association". Empty string browses the query roots.'),
        max_results: z
            .number()
            .int()
            .positive()
        ...
        try {
            ...
            const text = searchTrimmedIntrospection(cache.introspection, input.query ?? "", input.max_results ?? 12);
            return {
                content: [{ type: "text", text }],
                structuredContent: {
                    success: true,
            ...
            return createCodeModeError(ErrorCodes.UNKNOWN_ERROR, `${prefix}_search failed: ${message}`);
        }
    });
}
//# sourceMappingURL=graphql-schema-discovery.js.map