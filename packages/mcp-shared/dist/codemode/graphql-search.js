...
function formatField(typeName, field) {
...
function queryRootFields(intro) {
...
function renderQueryRoots(intro, max) {
    const roots = queryRootFields(intro).slice(0, max);
    if (roots.length === 0)
        return "No query-root fields found in the schema.";
    const lines = roots.map((f) => `  • ${f.name}${formatArgs(f)}: ${f.type}`);
    return (`Query roots (top-level entry points) — call one of these inside a gql.query('{ … }'):\n` +
        lines.join("\n") +
 ...
 */
export function searchTrimmedIntrospection(intro, query, maxResults = 12) {
    const tokens = query.toLowerCase().split(/\s+/).filter(Boolean);
    if (tokens.length === 0)
        return renderQueryRoots(intro, maxResults);
    ...
    for (const t of intro.types) {
        ...
        for (const tok of tokens) {
            if (typeName.includes(tok))
                typeScore += 3;
            if (typeDesc.includes(tok))
                typeScore += 1;
        }
        if (typeScore > 0)
            typeMatches.push(t.name);
        for (const f of t.fields ?? []) {
            const fieldName = f.name.toLowerCase();
            const fieldDesc = (f.description || "").toLowerCase();
            let fieldScore = 0;
            for (const tok of tokens) {
                if (fieldName.includes(tok))
                    fieldScore += 2;
                if (fieldDesc.includes(tok))
                    fieldScore += 1;
            }
            if (fieldScore > 0) {
                fieldHits.push({
    ...
    fieldHits.sort((a, b) => Number(b.isQueryRoot) - Number(a.isQueryRoot) || b.score - a.score);
    if (fieldHits.length === 0 && typeMatches.length === 0) {
        return (`No schema matches for "${query}". Browse the entry points with an empty query, ` +
    ...
    const out = [`${fieldHits.length} schema match(es) for "${query}":`];
    if (roots.length > 0) {
...
//# sourceMappingURL=graphql-search.js.map