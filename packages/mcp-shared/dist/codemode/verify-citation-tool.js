 ...
 */
import { z } from "zod";
import { canonicalJson, sha256Hex, verifyResultHash, } from "../provenance/provenance";
import { importCitationPublicKey, verifyCitationSignature, } from "../provenance/signing";
import { createCodeModeError, createCodeModeResponse, ErrorCodes, } from "./response";
const TOOL_NAME = "verify_citation";
const DESCRIPTION = "Re-check a citation's integrity anchors. Result integrity: pass " +
...
async function runChecks(input) {
    ...
    if (isNonEmptyString(input.expected_hash)) {
        checks.result_check = await verifyResultHash(input.expected_hash, input.data);
    }
 ...
 */
async function runSignatureCheck(input) {
    if (input.citation === undefined || input.public_jwk === undefined) {
        return undefined;
    }
    try {
        const key = await importCitationPublicKey(input.public_jwk);
        return await verifyCitationSignature(input.citation, key);
    }
    catch {
 ...
 */
export function structuralDrift(baseline, data) {
    const a = new Map();
    const b = new Map();
    ...
    removed.sort();
    changed_paths.sort();
    const truncated = added.length > DRIFT_PATH_CAP ||
 ...
 */
async function computeDrift(input) {
    ...
    const report = { ...summary, baseline_hash };
    if (isNonEmptyString(input.expected_hash)) {
        report.baseline_matches_expected = baseline_hash === input.expected_hash;
    }
    return report;
}
function describeCheck(label, check) {
    if (!check)
        return "";
...
function replayHintFor(checks, hasBaseline) {
    if (checks.query_check?.verified && !checks.result_check) {
        return ("Query identity confirmed — to finish adjudication, re-run this exact code via the " +
    ...
    if (checks.result_check && !checks.result_check.verified && !hasBaseline) {
        return ("Result mismatch — this is drift or a bad citation. To see WHAT changed, pass the " +
 ...
 */
function describeSignature(verdict) {
    if (!verdict)
        return "";
    if (verdict.verified) {
...
function buildPayload(checks, drift, signature_check) {
    const present = [checks.result_check, checks.query_check].filter((c) => c !== undefined);
    ...
    if (checks.query_check)
        payload.query_check = checks.query_check;
    if (drift)
        payload.drift = drift;
    if (signature_check)
        payload.signature_check = signature_check;
    if (replay_hint)
        payload.replay_hint = replay_hint;
    const textSummary = (verified ? "Verified." : "NOT verified.") +
...
function buildSchema() {
    return {
        expected_hash: z
            .string()
            .optional()
            .describe("A previously-issued result_hash (hex sha256) to verify `data` against — " +
        ...
        query_hash: z
            .string()
            .optional()
            .describe("A previously-issued query_hash (hex sha256) to verify `query` against — " +
            "e.g. Citation.query_hash. Enables the replay protocol."),
        query: z
            .unknown()
            .optional()
            .describe("The exact query to re-hash: for <prefix>_execute citations this is the raw " +
            "code STRING that was executed; for other tools, the args value."),
        baseline: z
            .unknown()
            .optional()
            .describe("Optional drift analysis: a previously-seen (originally-cited) result to diff " +
        ...
        citation: z
            .unknown()
            .optional()
            .describe("Attestation mode: the full Citation object (from a result's _meta.citation) " +
            "whose Ed25519 signature should be verified. Pair with `public_jwk`."),
        public_jwk: z
            .unknown()
            .optional()
            .describe("Attestation mode: the issuing server's public JWK (one entry from its " +
...
async function handle(input) {
    ...
    if (!checks.result_check && !checks.query_check && !signature_check) {
        return createCodeModeError(ErrorCodes.INVALID_ARGUMENTS, "Provide at least one of: { expected_hash, data } (result integrity), " +
