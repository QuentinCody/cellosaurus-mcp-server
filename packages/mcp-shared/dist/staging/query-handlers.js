...
async function workspaceQuery(route, args, toolPrefix) {
    try {
        ...
        const detail = await enrichStagedQueryError(msg, () => getWorkspaceSchemaFromDo(route.namespace, route.id));
        return createCodeModeError("SQL_EXECUTION_ERROR", `${toolPrefix}_query_data failed: ${detail}`);
    }
}
...
async function perServerQuery(doNamespace, args, toolPrefix) {
    try {
        ...
        const detail = await enrichStagedQueryError(msg, () => getSchemaFromDo(doNamespace, String(args.data_access_id || "")));
        return createCodeModeError(queryErrorCode(msg, err), `${toolPrefix}_query_data failed: ${detail}`);
    }
}
/**
