...
export const CITATION_SIG_ALG = "Ed25519";
 ...
 */
export function citationSigningInput(citation, signedAt) {
 ...
 */
export async function signCitation(citation, signer, signedAt) {
    const input = new TextEncoder().encode(citationSigningInput(citation, signedAt));
    const raw = await crypto.subtle.sign({ name: CITATION_SIG_ALG }, signer.privateKey, input);
    const signature = {
 ...
 */
export async function verifyCitationSignature(citation, publicKey) {
    const sig = citation.signature;
    if (!sig)
        return { verified: false, reason: "unsigned" };
    if (sig.alg !== CITATION_SIG_ALG ||
    ...
    try {
        const input = new TextEncoder().encode(citationSigningInput(citation, sig.signed_at));
        const ok = await crypto.subtle.verify({ name: CITATION_SIG_ALG }, publicKey, base64urlToBytes(sig.sig), input);
        return ok
...
export async function importCitationPrivateKey(jwk) {
    return crypto.subtle.importKey("jwk", coreJwk(jwk), { name: CITATION_SIG_ALG }, false, ["sign"]);
}
/** Import a public JWK for verification. */
export async function importCitationPublicKey(jwk) {
    return crypto.subtle.importKey("jwk", coreJwk(jwk), { name: CITATION_SIG_ALG }, true, ["verify"]);
}
/** Export a public key as a JWKS-ready JWK, stamped with `kid`. */
export async function exportCitationPublicJwk(publicKey, keyId) {
