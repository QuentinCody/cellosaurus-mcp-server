...
function extractGuardInfo(result) {
    if (result === null || typeof result !== "object")
        return undefined;
    const direct = result.__guard;
    if (direct && typeof direct === "object") {
        return direct;
    }
    const nested = result.data?.__guard;
 ...
 */
function classifyNegative(result, recordCount) {
...
function formatNegativeSuffix(verification) {
...
function formatCitation(source, tool, retrievedAt, resultHash, recordCount, verification) {
...
export async function buildCitation(input) {
    ...
    return {
        ...
        ...(input.dataAccessId !== undefined
            ...
            : {}),
        ...(negative ?? {}),
        text: formatCitation(input.source, input.tool, input.retrievedAt, result_hash, input.recordCount, negative?.verification),
    };
}
/**
